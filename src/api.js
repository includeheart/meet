...
const getToken = async (code) => {
  ...
  const response = await fetch(
    'https://tcb2tom6b3.execute-api.eu-central-1.amazonaws.com/dev/api/token' + '/' + encodeCode
  );
...
export const getAccessToken = async () => {
 ...
 if (!accessToken || tokenCheck.error) {
   ...
   if (!code) {
     const response = await fetch(
       "https://tcb2tom6b3.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url"
     );
     const result = await response.json();
